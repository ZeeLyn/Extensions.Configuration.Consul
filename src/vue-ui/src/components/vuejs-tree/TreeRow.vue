    ...
    >
      ...
      >
        ...
        <i
          ...
          :class="[{'expanded': expanded == true}, styles.expanded.class, 'icon-expanded']"
        ></i>
        <span
          v-else-if="options.events.expanded.state == true && (node.nodes == undefined || node.nodes.length==0)"
...
export default {
  ...
  methods: {
    toggleEvent: function(eventType, node) {
      if (
        eventType == "editableName" &&
        this.options.events["editableName"].calledEvent
      ) {
    ...
    recCallNodes: function(state, event, nodes) {
      ...
      nodes.forEach(function(node) {
        if (!node.state)
          node.state = { checked: false, expanded: false, selected: false };
